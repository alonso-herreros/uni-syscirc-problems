...
def evaluate_on_range(f, time:list[float], var:Symbol=Symbol("t"), **kwargs):
    ...
    for i in time:
        ...
        try: val = float(Known.resolve(val, levels=-1, evalf=True))
            ...
            try: raise ValueError(f"Could not evaluate {val} to float. Symbols left: {val.free_symbols}.") from e
            except TypeError: raise ValueError(f"Could not evaluate {val} to float.") from e
